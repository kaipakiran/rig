...
// Streaming Op trait
// ================================================================
pub trait StreamingOp: Send + Sync {
    type Input: Send + Sync;
    type Output: Send + Sync;
    type Stream: Stream<Item = Self::Output> + Send;

    fn stream(&self, input: Self::Input) -> BoxFuture<'static, Self::Stream>;

    /// Chain a function `f` to the current streaming op.
    fn map<F, Input>(self, f: F) -> StreamingSequential<Self, StreamingMap<F, Self::Output>>
    where
        F: Fn(Self::Output) -> Input + Send + Sync + Clone,
        Input: Send + Sync,
        Self: Sized,
    {
        StreamingSequential::new(self, StreamingMap::new(f))
    }

    /// Chain an asynchronous function `f` to the current streaming op.
    fn then<F, Fut>(self, f: F) -> StreamingSequential<Self, StreamingThen<F, Fut::Output>>
    where
        F: Fn(Self::Output) -> Fut + Send + Sync + Clone,
        Fut: Future + Send + Sync,
        Fut::Output: Send + Sync,
        Self: Sized,
    {
        StreamingSequential::new(self, StreamingThen::new(f))
    }

    /// Chain another streaming operation to the current op.
    fn chain<T>(self, op: T) -> StreamingSequential<Self, T>
    where
        T: StreamingOp<Input = Self::Output> + Clone,
        Self: Sized,
    {
        StreamingSequential::new(self, op)
    }
}

// ================================================================
// Streaming Op implementations
// ================================================================
pub struct StreamingSequential<Op1, Op2> {
    prev: Op1,
    op: Op2,
}

impl<Op1, Op2> StreamingSequential<Op1, Op2> {
    pub(crate) fn new(prev: Op1, op: Op2) -> Self {
        Self { prev, op }
    }
}

impl<Op1, Op2> StreamingOp for StreamingSequential<Op1, Op2>
where
    Op1: StreamingOp + 'static,
    Op2: StreamingOp<Input = Op1::Output> + Clone + 'static,
    Op1::Stream: Send,
    Op2::Stream: Send,
{
    type Input = Op1::Input;
    ...
    fn stream(&self, input: Self::Input) -> BoxFuture<'static, Self::Stream> {
        ...
        let prev_future = self.prev.stream(input);
        Box::pin(async move {
            let prev_stream = prev_future.await;
            Box::pin(prev_stream.flat_map(move |output| {
                let op = op.clone();
                async_stream::stream! {
                    let stream = op.stream(output).await;
                    for await item in stream {
                        yield item;
                    }
                }
            })) as Pin<Box<dyn Stream<Item = Self::Output> + Send>>
        })
    }
}

pub struct StreamingMap<F, Input> {
    f: F,
    _t: std::marker::PhantomData<Input>,
}

impl<F: Clone, Input> Clone for StreamingMap<F, Input> {
    fn clone(&self) -> Self {
        Self {
            f: self.f.clone(),
            _t: std::marker::PhantomData,
...
impl<F, Input> StreamingMap<F, Input> {
    pub(crate) fn new(f: F) -> Self {
        Self {
            f,
            _t: std::marker::PhantomData,
...
impl<F, Input, Output> StreamingOp for StreamingMap<F, Input>
where
    ...
    Output: Send + Sync + 'static,
{
    ...
    fn stream(&self, input: Self::Input) -> BoxFuture<'static, Self::Stream> {
        let f = self.f.clone();
        Box::pin(async move {
            Box::pin(stream::once(futures::future::ready((f)(input)))) as Pin<Box<dyn Stream<Item = Self::Output> + Send>>
        })
    }
}

pub struct StreamingThen<F, Input> {
    f: F,
    _t: std::marker::PhantomData<Input>,
}

impl<F, Input> StreamingThen<F, Input> {
    pub(crate) fn new(f: F) -> Self {
        Self {
            f,
            _t: std::marker::PhantomData,
...
impl<F, Input, Fut> StreamingOp for StreamingThen<F, Input>
where
    ...
    Fut::Output: Send + Sync + 'static,
{
    ...
    fn stream(&self, input: Self::Input) -> BoxFuture<'static, Self::Stream> {
        ...
        Box::pin(async move {
            let fut = (f)(input);
            Box::pin(stream::once(fut)) as Pin<Box<dyn Stream<Item = Self::Output> + Send>>
        })
    }
}

pub struct StreamingPassthrough<T> {
    _t: std::marker::PhantomData<T>,
}

impl<T> StreamingPassthrough<T> {
    pub(crate) fn new() -> Self {
        Self {
            _t: std::marker::PhantomData,
        }
...
impl<T> StreamingOp for StreamingPassthrough<T>
where
    T: Send + Sync + Clone + 'static,
{
    ...
    fn stream(&self, input: Self::Input) -> BoxFuture<'static, Self::Stream> {
        Box::pin(async move {
            Box::pin(stream::once(async move { input })) as Pin<Box<dyn Stream<Item = Self::Output> + Send>>
        })
    }
}
